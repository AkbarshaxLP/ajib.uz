<template>
  <section class="keyspecs-section">
    <div class="container">
      <div class="keyspecs-inner" v-scroll-animate="'fade-up'">
        ...
        <div class="phone-col">
          <img
            v-if="imgOk"
            src="https://ajib.uz/wp-content/uploads/2025/02/1.png"
            alt="ajib i25"
            class="phone-img"
            @error="imgOk = false"
          />
          <div v-else class="phone-placeholder">ajib i25</div>
        </div>
        ...
        <div class="specs-col">
          ...
          <div class="specs-grid">
            <div v-for="spec in SPECS" :key="spec.label" class="spec-card">
              <div class="spec-icon">
                <component :is="spec.icon" />
              </div>
              <span class="spec-text">{{ spec.label }}</span>
            </div>
          </div>
        </div>
...
const SPECS = [
  { label: 'Displey: 6.7" HD+ IPS 90Hz', icon: 'IconParamsIconDisplay'   },
  { label: 'Bluetooth: 5.0',              icon: 'IconParamsIconBluetooth' },
  { label: 'Xotira: 6+6 GB / 128 GB',    icon: 'IconParamsIconMemory'    },
  { label: 'Batareya: 5000 mAh',          icon: 'IconParamsIconBattery'   },
  { label: 'Kamera: 48 MP',               icon: 'IconParamsIconCamera'    },
  { label: 'Protsessor: Unisoc T606',     icon: 'IconParamsIconProcessor' },
]
</script>
